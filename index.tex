...
octobre 2025}};
      \node [anchor = east] at (8.5,-12){\textcolor{black}{Modifié le : 13
novembre 2025}};
...
Askenazy, Cette et Sylvain (2012), Piton (2019), Cette, Koehl et
Philippon (1996), Timbeau (2025), Gendre et Thommen (2025)). Un
...
et de sa comparabilité entre pays.
...
\item
  ...
  Philippon (1996) et Gendre et Thommen (2025). Nous procédons à cette
  ...
  masse salariale supérieur au revenu mixte des non salariés\footnote{Merci
    à E. Aurissergues pour m'avoir pointé cette anomalie.}. En France,
  le revenu mixte (des non-salariés) est passé de 30\% de la masse
  salariale (autant pour le ratio des effectifs) au début des années
  1970 à un peu moins de 10\% (presque 15\% pour les effectifs) en 2024.
  Malheureusement, le revenu mixte par branche n'est pas diffusé
  systématiquement par les Etats membres de l'UE. Ces corrections
  modifient significativement la part des salaires dans la valeur
  ajoutée, suivant la méthode, parce que non seulement la part des non
  salariés et de leur revenu mixte varie dans le temps, de façon
  ...
  différente par pays (graphique~\ref{fig-psalcnc} et voir les annexes D
  et G consacrées à ce point).
\item
  \textbf{Définir le champ considéré}. Il est plus facile de faire le
  calcul au niveau le plus agrégé, mais ce champ inclut les branches non
  ...
  propriétaires) et pour part produits par les ménages pour d'autres
  ménages (les propriétaires bailleurs). Suivant les pays ou les
  périodes, le nombre de propriétaires occupants peut être très
  différent (Cette, Koehl et Philippon (1996) fait ce point que Gendre
  et Thommen (2025) semble ignorer). Mais le vrai problème est que le
  partage de la valeur ajoutée n'a ici pas de sens dansle cas d'un loyer
  imputé ou d'un propriétaire bailleur. La consommation de services
  immobiliers par les ménages représentait en France en 2024 un peu
  moins de 300~milliards d'euros soit aprroximativement 18\% de la
  valeur ajoutée brute hors services immobilier. On comprend donc que ce
  point n'est pas du tout négigeable. La notion privilégiée est donc
  celle de partage de la valeur ajoutée dans les branches marchandes
  hors services immobiliers ou, de façon plus précise, en enlevant de la
  valeur ajoutée marchande la branche «\,services immobiliers (L)\,»
...
\item
  \textbf{Utiliser la notion de valeur ajoutée nette} (de la
  consommation de capital fixe, la CCF) plutôt que brute. Rappelons que
  la valeur ajoutée nette est construite en ôtant de la valeur ajoutée
  brute la consommation de capital fixe. Cette dernière notion découle
  de l'application de tables de mortalité à un inventaire permanent des
  ...
  valeur ajoutée (graphique~\ref{fig-psalnetbrut}). Ce point est signalé
  par Gendre et Thommen (2025), mais les auteurs ont préféré le concept
  de valeur ajoutée brute pour des raisons de comparabilité entre
  études\,-- au détriment de la comparabilité dans le temps et dans
  l'espace.
\end{itemize}
...
personne ou le revenu mixte par branche)\footnote{Cette, Koehl et
  Philippon (1996) utilisent une définition plus large des branches non
  marchande en incluant les branches R, T et U.} :
...
des pays sélectionnés (correction des non salariés par les effectifs),
pour autant que l'on puisse comparer entre pays. Ce résultat contraste
avec les évaluations récentes de Cette, Koehl et Philippon (1996) et
Gendre et Thommen (2025) qui concluent à une stabilité de la part des
salaires dans la valeur ajoutée autour de ce qu'ils supposent être une
valeur d'équilibre.

Théoriquement\footnote{Cette, Koehl et Philippon (1996) propose une
...
\subsection{Rendement financier du capital productif\,: la France au
plus
bas}\label{rendement-financier-du-capital-productif-la-france-au-plus-bas}
...
net et le stock de capital excluent toutes les activités immobilières
...
la nomenclature de la NACE rev. 2). Dans quelques pays, une part
...
suggère que la séparation branche/secteur n'est pas complètement assurée
ou a été interprétée assez librement.

La France occupe une position singulière avec un rendement du capital
...
salariale des branches services immobiliers et (surtout) services
financiers est plus élevée que la masse salariale dans les autres
branches marchandes.
...
salariés non salariés au niveau des branches.

D'après l'INSEE, (voir le blog
...
Le développement des plateformes et dans certains pays de statuts
(sociaux, fiscaux) particuliers (les micro-entrepreneurs\,-- ou
auto-entrepreneurs en France) a introduit une nouvelle «\,classe\,» de
...
employée en utilisant les données de revenu mixte. A défaut d'être
...
d'un pays à l'autre et surtout occultent un pan important de l'économie
...
rev. 2 \hyperref[0]{\texttt{nama\_10\_a64}}), agrégée en 9 branches,
puis extrapolée pour les années non connues (ici 2024 et 2025) en
conservant un ratio constant dans la valeur ajouté brute. Le détail se
trouve dans le code.
...
loyers imputés sont comptabilisés comme une valeur ajoutée des ménages).
...
de l'année (ici 2 ou 3 trimestres sur 4) susceptible de changer au fur
et à mesure du temps. Il est possible en modifiant le code de produire
un graphique trimestriel ou trimestriel lissé.
...
En France et en Belgique, la convention employée surestime visiblement
...
dynamique en généralisant le nombre de non salariés ayant de faibles
...
du revenu mixte\footnote{Le chiffre de 88\% est établi à partir des
  ...
  souple.}, le reste pour approcher la CCF des indépendants\,-- et non
le nombre de non salariés (correction par les effectifs) comme base de
la correction.
...
moins précise pour l'Allemagne puisque nous n'avons pas les données de
revenu mixte par branches, contrairement à la France.
...
réduire le dénominateur (la valeur ajoutée) de la consommation de
...
marginalement modifié, la Belgique ayant une part des salaires nette
plus élevée que l'Allemagne, alors que sa part brute est plus faible
qu'en Allemagne. Pour les autres pays, le classement est identique (La
...
sur le même champ (\emph{i.e.} en enlevant la sous branche L68A), on
peut estimer un rendement du capital productif.
...
et de la valorisation des actifs\footnote{Une alternative serait de
  disposer de l'impôt sur les sociétés (\$D51\$) par branche, mais le
  passage branche produits empêche de le faire simplement.}. Dans
l'approche comptes d'agents (ou de secteurs institutionnels), on affecte
la valeur nette résiduelle des entreprises au stock de capital. De plus,
le stock de capital n'est pas dans l'approche du
graphique~\ref{fig-tprofitsnff} limité au capital productif mais intègre
également des actifs financiers qui n'ont pas de contrepartie physique
parce qu'ils sont hors territoire français.
...
comptes des entreprises. Cette méthode met de côté une part importante
de l'activité, puisque les entités économiques qui ne sont pas des
entités légales ne sont pas prises en compte.
...
En revanche, Eurostat a un programme de coopération (avec l'OCDE entre
autres) pour intégrer les données dans le cadre de données d'Eurostat
(sans que les méthodes ne soient parfaitement homogènes pour autant).
...
l'activité résidentielle, et sa partie auto-produite, sont intégrées
dans la branche «\,services immobiliers\,». A partir des données
détaillées de consommation (base
...
\fontsize{9.0pt}{11.0pt}\selectfont
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}>{\raggedright\arraybackslash}p{\dimexpr 90.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\raggedleft\arraybackslash}p{\dimexpr 75.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\raggedleft\arraybackslash}p{\dimexpr 75.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\raggedleft\arraybackslash}p{\dimexpr 75.00pt -2\tabcolsep-1.5\arrayrulewidth}>{\raggedleft\arraybackslash}p{\dimexpr 75.00pt -2\tabcolsep-1.5\arrayrulewidth}}
\toprule
 & \multicolumn{4}{>{\centering\arraybackslash}m{\dimexpr 315.00pt -2\tabcolsep-1.5\arrayrulewidth}}{Part dans la VAB marchande en 2024 des} \\ 
\cmidrule(lr){2-5}
 & services immobiliers & services immobiliers hors ménages & loyers des ménages & loyers imputés aux ménages \\ 
\midrule\addlinespace[2.5pt]
...
\end{tabular*}
\begin{minipage}{\linewidth}
\emph{Source} : Eurostat Household final consumption expenditure by purpose (nama\_10\_cp18)\\
\end{minipage}
...
Cette G., Koehl L., Philippon T. (1996).
