...
L'organisation est assez standard. Le fichier `qmd` principal est `index.qmd`, les codes produisant les données sont dans `R`. Les fichiers `R` sont exécutés par `ofce::source_data()` qui exécute en cachant les résultats d'exécution. Le répertoire `data` contient quelques fichiers téléchargés (par les scripts), `divers` contient des éléments annexes, `ecopol` le code source de l'article d'Economie Politique.
