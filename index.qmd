...
thanks: "L'auteur remercie chaleureusement Elliot Aurissergues, Francçois Geerolf et Eric Heyer pour leurs remarques stimulantes. Elles m'ont amené à rendre plus complexe l'exploration des concepts mais la confusion qui en découle est de ma seule responsabilité. "
---
...
nsal |> 
  ...
  fmt_number(c(r), decimals = 0, scale_by = 100, dec_mark = ",", sep_mark = " ", pattern = "{x} %") |> 
  cols_width(c(nsal, b3g, r) ~ px(120)) |> 
...
On peut construire à partir de ces éléments une correction alternative de la non salarisation en utilisant le revenu mixte – en retenant 80% du revenu mixte, le reste pour approcher la CCF – et non le nombre de non salariés comme base de la correction.
...
data <- naq |> 
  ...
  select(time, geo, champ, psal, psalm) |> 
...
psal <- ggplot(data) +
  aes(x=time, col = geo) +
  ggbraid::geom_braid(
    data = naq |> filter(champ %in%c("mdhi")) |> mutate(s = psal>psalm),
 ...
 comptes nationaux trimestriels (namq_10_a10), téléchargés le {date}, code à github.com/xtimbeau/travail/vaq.r.",
    ...
    note = "La correction par le revenu mixte n'est pas exacte pour le champ considéré, sauf pour la France.")

girafy(psal, 1.5)
```

La correction est assez importante pour la France et la Belgique, ce qui est conforme aux valeurs affichées dans le @tbl-mixte.
...
La France conserve une singularité marquée par la baisse continue du taux de profit au cours du temps. Le rendement apparent du capital est ainsi très bas, plus bas que dans tous les autres pays considérés où il est plutôt stable (l'Italie fait exception avec une forte volatilité). Cette singularité subsiste lorsqu'on utilise la correction de la non salarisation par le revenu mixte au lieu des comptes de branches (@fig-rpmixte). La position de la France est un peu moins singulière, mais le diagnostic subsiste.
...
Plusieurs explications peuvent être avancées à la dégradation du rendement du capital productif[^1] :

[^1]: On peut aussi mettre en cause la qualité des comptes nationaux et leur incapacité à apporter une information même bruitée sur la réalité des économies contemporaines. Cette explication est commode lorsqu'on a une théorie envers laquelles les "faits" comptables sont têtus. Nous partons du principe qu'on peut
...
3 annexes (domaine des données, CCF et comparaisons entre pays) sont accessibles en ligne à l'adresse <https://xtimbeau.github.io/travail/>
:::
