...
psal <- imap(
 ...
 comptes nationaux trimestriels (namq_10_a10), téléchargés le {date}, code à github.com/xtimbeau/travail/R/vaq.r.",
    champ = "{champs[.x]}.",
    note = "Part des salaires dans la Valeur Ajoutée Nette (brute corrigée de la CCF), correction des non salariés par le revenu mixte (hybride pour les pays autre que la France). 2025 est une extrapolation sur la base d'une information partielle jusqu'au T3. Les traits colorés correspondent à chaque pays, les traits fins gris rappellent les autres pays.") ) 
...
### Rendement financier du capital productif : la France au plus bas
...
On peut construire à partir du revenu mixte une correction alternative de la non salarisation en utilisant le revenu mixte – en retenant 88% du revenu mixte[^4], le reste pour approcher la CCF des indépendants – et non le nombre de non salariés (correction par les effectifs) comme base de la correction.
...
naq2 <- source_data("R/vaq.r")$naa |> 
  ...
  mutate(psalh = ifelse(is.na(psalm.fr), psalh, psalm.fr)) |>
...
psal <- ggplot(data) +
  aes(x=time, col = geo) +
  ggbraid::geom_braid(
    data = naq2 |> mutate(s = psal>psalh),
    aes(ymin = psal, ymax = psalh, group = geo, fill = s), 
    color = NA, alpha=0.1, show.legend = FALSE) +
  scale_fill_manual(values = c("green", "red")) +
  ggnewscale::new_scale_fill() +
  scale_alpha_manual(
    values = c(psal=1, psalh = .75),
    labels = labels,
    name = NULL) +
  scale_linewidth_manual(
    values = c(psal=.5, psalh = 0.25),
    labels = labels,
    name = NULL) +
  geom_line_interactive( 
...
[^5]: Une alternative serait de disposer de l'impôt sur les sociétés (\$D51\$) par branche, mais le passage branche produits empêche de le faire simplement.
...
tpmixte <- ggplot(data_long) +
  ...
  scale_fill_manual(values = c("green", "red")) +
  ggnewscale::new_scale_fill() +
  scale_linetype_manual(values = c(rbh="11", rb="solid"),
                        name=NULL, 
...
## L'optimisation fiscale par l'immobilier {#sec-immobilier}
...
-   un autre mécanisme d'optimisation fiscale, par la séparation des activités productives et des locaux qui leur sont nécessaires. Les loyers sont alors un prix de transfert et permettraient de bénéficier de la fiscalité avantageuse de l'immobilier[^7].
