...
-   Corriger des non salariés et leur imputer une masse salariale. Cette correction est standard. Elle repose sur des hypothèses importantes comme le salaire affecté à un non salarié. Nous utilisons la décomposition en branches (ou en secteurs dans certains pays) pour affecter aux non salariés d'une branche le salaire moyen des salariés de cette branche. C'est une hypothèse assez forte, pour laquelle nous proposons une alternative, en utilisant la notion de revenu mixte. Malheureusement, le revenu mixte par branche – qui est important pour le point suivant – n'est pas diffusé systématiquement par les Etats membres de l'UE (en fait seule la France diffuse cette donnée). Cette correction a des conséquences importante pour les comparaisons entre périodes pour un même pays parce que non seulement la part des non salariés varie dans le temps, de façon différente suivant les branches et entre pays (@fig-psalcnc) mais aussi la rémunaration des non salariés varie dans le temps (voir l'annexe D consacrée à ce point).
...
psaleu <- source_data("ggvanettebrute.r")
tabsetize(psaleu$gg)
```
...
vroom::vroom_write(data4tp, "data4tp.csv")
...
psal_fr <- source_data("france-melodi.r")$aggr |> 
  transmute(time, geo = "FR", champ, psal, psalm)

naq2 <- source_data("vaq.r")$naa |> 
  filter(geo %in% P6) |> 
  mutate(tooltip = glue(
    "<b>{lbl(geo)}</b>
    {year(time)} 
...
psal <- ggplot(data) +
  ...
  ofce_caption(
    source = "Eurostat, comptes nationaux annuels (nama_10_a64, nama_10_a64_e),
 comptes nationaux trimestriels (namq_10_a10), téléchargés le {date}, INSEE activités des branches, CN2020, code à github.com/xtimbeau/travail/vaq.r et france-medoli.fr",
...
psal_fr <- source_data("france-melodi.r")$aggr |> 
  ...
  left_join(source_data("vaq.r")$naaa |>
              filter(geo == "FR") |>
...
Plusieurs explications peuvent être avancées à la dégradation du rendement du capital productif[^2] :

[^2]: On peut aussi mettre en cause la qualité des comptes nationaux et leur incapacité à apporter une information même bruitée sur la réalité des économies contemporaines. Cette explication est commode lorsqu'on a une théorie envers laquelles les "faits" comptables sont têtus. Nous partons du principe qu'on peut
...
data <- source_data("france-melodi.r") |> 
  pluck("aggr") |>
...
```{r}
doms <- source_data("vaq.r")$dom
pays <- source_data("nace.r")$pays3
emp_dom <- source_data("naq10_e.r")$dom
assets_dom <- source_data("assets.r")$dom
meta <- sourcoise::sourcoise_meta("vaq.r")
date <- meta$data_date |> ymd_hms() 
date <- stringr::str_glue("{day(date)}/{month(date)}/{year(date)}")
...
```{r, results="asis"}
#| fig-asp: 0.85 
PA <- source_data("nace.r")$pays2 
anD <- source_data("annexeD.r") |> 
  mutate(
    reff = self/sal,
    reffh = selfh/salh,
    rb3g = b3g/d1, 
...
gD <-imap(
  ...
  ofce_caption(
    ...
    source = "Eurostat, comptes nationaux annuels de secteur (nasa_10_nf_tr), emploi (nama_10_a64_e), EU-SILC (ilc_di05), téléchargés le {date}, code à github.com/xtimbeau/travail/annexeD.r.",
...
gD2 <-imap(
  ...
  scale_y_continuous(
    ...
    theme(legend.direction = "horizontal") +
  ofce_caption(
    source = "INSEE, téléchargés le {date}, code à   github.com/xtimbeau/travail/annexeD.r.",
 champ = "voir légende") 
  })
...
### Annexe E : Base 2020 et 2014 depuis 1949 {.unnumbered}

::: {#fig-psalfr49}
```{r, results="asis"}
#| fig-asp: 0.85

cn14 <- source_data("france CN 2014.R") |> 
  select(time, champ, psal, starts_with("na"))
cn20 <- source_data("france-melodi.r")$aggr |> 
...
data <- full_join(cn14, cn20, by=c("time", "champ"), suffix = c(".14", ".20")) |>
  mutate(
    ...
    tooltip = glue(
      ...
      CNA 2020 : {round(psal.20,1)}%
      CNA 2014 : {round(psal.14,1)}%" ) ) |> 
  pivot_longer(c(psal.20, psal.14))
...
psal <- imap(
  ...
  ~ggplot(data |> filter(champ == .x)) +
    ...
    PrettyCols::scale_color_pretty_d(
      "Joyful",
      name = NULL,
      labels = c(psal.14 = "CNA 2014", psal.20 = "CNA 2020")) +
    theme(legend.direction = "horizontal") +
...
4 annexes (domaine des données, CCF, comparaisons entre pays et rémunération des non salariés) sont accessibles en ligne à l'adresse <https://xtimbeau.github.io/travail/>
:::
