...
data <- salaires |> 
  ...
  select(geo, time, wr, wr_md, wr_nmd) |> 
  pivot_longer(c(wr, wr_md, wr_nmd)) |> 
  mutate(
    nlabel = case_match(
      name, 
...
sal <- ggplot(data) +
  ...
  scale_linetype_manual(values = c(wr="solid", wr_md ="solid", wr_nmd ="dotted")) +
  scale_linewidth_manual(values = c(wr=.5, wr_md = 0.1, wr_nmd = 0.1)) +
  ...
  ofce_caption(
    wrap = 0,
    source = "Eurostat, comptes nationaux trimestriels (namq_10_a10), téléchargé le {date},
    calcul de l'auteur.",
    dpt = salaires$time,
    dptf = "quarter",
...
psaloecd <- ggplot(data |> filter(year(time)>=1995)) +
  ...
  geom_point_interactive(
    aes(tooltip = tooltip, data_id = time, fill = geo),
    size = .01, stroke = 0., shape = 21, color = "white",
    hover_nearest = TRUE, show.legend=FALSE) +
