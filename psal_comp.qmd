...
psal <- imap(c(mdhi = "Hors services immobiliers (-L)", mdhim = "Hors loyers imputés (-L68A)"),
  ~ggplot(naq |> filter(champ == .y)) +
    aes(x=time, y=psal, col = geo, group = geo) +
    geom_line(color = "grey85", layout = "fixed", linewidth = 0.2) +
    geom_line_interactive(
      aes(tooltip = "Part des salaires dans la VAN,<br>corrigée de la non salarisation,<br>{.x}" |> glue()),
      linewidth = 0.5) +
 ...
 comptes nationaux trimestriels (namq_10_a10), téléchargés le {date}.",
    ...
    champ = "Branches marchandes {tolower(.x)}.",
