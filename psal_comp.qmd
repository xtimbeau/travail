...
psal <- imap(
  c("hors services immobiliers (-L)" = "mdhi", 
    "Marchand" = "md",
    "hors loyers imputés des ménages (-L68A)" = "mdhim"),
...
tpbranches <- imap( 
  c("Marchand-L" = "mdhi",
    ...
    " " = "md"), ~{
      ...
      ggplot(data) +
        aes(x=time, y=r, col = geo, group = geo) +
 ...
 comptes nationaux trimestriels (namq_10_a10), actifs productifs (nama_10_nfa_st), téléchargés le {date}, code à github.com/xtimbeau/travail/vaq.r.",
          ...
          note = "La CCF ou les impôts nets de subventions sont extrapolés au delà de 2023 ou 2024 à partir des comptes annuels (voir tableau A1). L'impôt sur les sociétés (D51) des sociétés financières (S12) est affecté à la branche K (services financiers) et celui des sociétés non financières (S11) aux branches marchandes hors services financiers et immobilier des ménages (K+L68A). Les traits colorés correspondent à chaque pays, les traits fins gris rappellent les autres pays.") })

girafy(tpbranches[[1]], 1.25)
```
...
doms$snq |>
  ...
  gt() |> 
  fmt_flag(geo) |> 
  tab_spanner(label = md("**B1G, D1**<br>trim., 17 br.<br>namq_10_a10"), columns = c(debq, finq)) |> 
  tab_spanner(label = md("**SAL, NSAL**<br>an., 88 br.<br>nama_10_a64_e"), columns = c(debs, fins)) |> 
  tab_spanner(label = md("**P51C, D29x39, D51**<br>an., 17 br.<br>nama_10_a64"), columns = c(deba, fina)) |> 
  tab_spanner(label = md("**L68A**<br>an., 88 br.<br>nama_10_a64"), columns = c(debl, finl)) |> 
  tab_spanner(label = md("**N11N**<br>an., 88 br.<br>nama_10_nfa_st"), columns = c(debk, fink)) |> 
  ...
  cols_width(
      ...
      geo_lbl ~ px(80),
      everything() ~ px(65)) |> 
  tab_source_note(md(glue("*Note* : données téléchargées sur le site d'Eurostat (package eurostat v4.0.0) le {date}, telles que publiées par Eurostat. Les instituts nationaux peuvent mettre à dispositon plus de données non (encore ?) prises en compte par Eurostat. Les pays sont classés par ordre de population décroissante. Les traits colorés correspondent à chaque pays, les traits fins gris rappellent les autres pays.")))
```
