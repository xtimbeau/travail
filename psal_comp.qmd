...
data <- naq_full |> 
  mutate(year = year(time)) |>
  filter( year %in% c(1995, 2025)) |>
  select(year, geo, psalb, vab) |> 
  pivot_wider(names_from = year, values_from = c(psalb,vab)) |> 
...
data_int <- data |> 
  ...
  mutate(
    tooltip = glue("<b>{lbl(geo)}</b>
                     Part des salaires dans la VAN en {year} : {round(100*psalb,1)}%
                     (VA brute, cor. non sal., marchandes hors imm. mén.)")) |> 
  ungroup()

library(ggimage)
psalbeu <- ggplot(data_int) + 
  geom_segment(
    aes(x = x,  xend = xend, y = y, yend = yend, color = year),
    key_glyph = draw_key_point, linewidth = 0.2) +
  ggimage::geom_flag( data = ~.x |> filter(i==1),
