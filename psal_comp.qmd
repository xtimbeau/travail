...
Au début des années 2000, deux pays se distinguaient des autres (l'Espagne et l'Italie) par une part des salaires plus faibles. L'écart avec l'Allemagne atteignait alors plus de 15 points. Moins développés, une part peut être plus importante de l'investissement expliquait cette situation. La généralisation de l'approche à d'autres pays de l'Union Européenne permet de confirmer cette hypothèse (@fig-psaleu). Aujourd'hui la situation est différente, l'écart entre l'Allemagne et ces pays s'étant drastiquement réduit (moins de 8 points entre l'Italie et l'Allemagne) et les Pays-Bas prenant la place du pays avancé où la part des salaires est la plus basse (avec l'exception du Luxembourg) et la France où elle est plus haute.
...
naq_full <- ofce::source_data("vaq.r")$naa |> 
  filter(geo != "IE", champ == "mdhim")
data <- naq_full |> 
  filter(geo != "IE") |> 
  mutate(year = year(time)) |>
  filter( year %in% c(1995, 2025)) |>
  select(year, geo, psal, van) |> 
  pivot_wider(names_from = year, values_from = c(psal,van)) |> 
...
data_int <- data |> 
  ...
  mutate(
    tooltip = glue("<b>{lbl(geo)}</b>
                     ...
                     (VAN, cor. non sal., marchandes hors imm. mén.)")) |> 
  ungroup()
...
psaleu <- ggplot(data_int) + 
  ...
  ggimage::geom_flag( data = ~.x |> filter(i==1),
                      aes(x= geo, y = 0.475, image = geo2c), size = .025)+
  ...
  geom_point_interactive(
    ...
    hover_nearest = TRUE,
    aes(x = geo, y=psal, tooltip = tooltip, color = year), size = 0.5, alpha = 0.5)+
  geom_point_interactive(
    data = ~.x |> filter(i%in%c(0,1)), 
