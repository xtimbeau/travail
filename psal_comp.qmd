...
-   Corriger des non salariés et leur imputer une masse salariale. Cette correction est standard. Elle repose sur des hypothèses importantes comme le salaire affecté à un non salarié. Nous utilisons la décomposition en branches (ou en secteurs dans certains pays) pour affecter aux non salariés d'une branche le salaire moyen des salariés de cette branche. C'est une hypothèse assez forte, mais qu'il est difficile de dépasser.

-   Etre soigneux sur le champ considéré. Il est plus facile de faire le calcul au niveau le plus agrégé, mais ce champ inclut les branches non marchandes dans lesquelles la notion de prix et donc de valeur ajoutée est parfois conventionnelle. Parmi les branches marchandes, la branches des services immobiliers est problématique parce qu'elle prend en compte la valeur ajoutée des ménages au travers des services immobiliers qui sont pour par auto produit (les loyers imputés aux propriétaires). La notion de partage de la valeur ajoutée n'a ici pas beaucoup de sens et la comparaison d'un pays à l'autre peut être très perturbée.

-   Utiliser la notion de valeur ajoutée nette plutôt que brute. Rappelons que la valeur ajoutée nette est construite en ôtant de la valeur ajoutée brute
...
sal <- ggplot(data) +
  aes(x=time, y=value, 
      ...
      color = geo) +
  scale_alpha_manual(values = c(wr=1, wr_md = 0.75, wr_nmd = 0.5, wr_hifi = 0.75)) +
  scale_linetype_manual(values = c(wr="solid", wr_md ="solid", wr_nmd ="solid", wr_hifi = "dotted")) +
  scale_linewidth_manual(values = c(wr=.5, wr_md = 0.1, wr_nmd = 0.25, wr_hifi = 0.5)) +
  geom_point_interactive(
    data = ~.x |> filter(name == "wr") |> group_by(geo) |> filter(quarter(time) == last(quarter(time))) |> ungroup(),
...
#| label: fig-sal
#| fig-asp: 1
#| fig-cap: Part des salaires dans la VA, comptes de branche, prise en compte de la non salarisation

champs  <- c("md" = "marchand", "mdhifi" = "marchand hors immobilier et services financiers",
            "mdhfi"= "marchand hors services financiers", "tb" = "Toutes branches (marchandes et non marchandes)")
naq <- source_data("vaq.r")$naa |> 
  mutate(tooltip = glue(
    "<b>{lbl(geo)}</b>
    {year(time)} {champs[champ]}
...
psal <- ggplot(naq |> filter(champ %in% c("md"))) +
  aes(x=time, y=psal, col = geo, linetype = champ, alpha = champ) +
  scale_alpha_manual(values = c(md=.75, mdhifi = 0.5)) +
  scale_linetype_manual(values = c(md="solid", mdhifi ="dotted")) +
  geom_ribbon(aes(ymin = psalnc, ymax = psal, fill = geo), 
              color = NA, alpha=0.1, show.legend = FALSE) +
  geom_line_interactive( 
    aes(y=psalnc, tooltip = "Part des salaires dans la VAN, non corrigée de la non salarisation"),
    show.legend=FALSE) +
  geom_line_interactive(
    aes(tooltip = "Part des salaires dans la VAN, corrigée de la non salarisation"),
    show.legend=FALSE) +
  geom_point_interactive(
    data = ~.x |> filter(champ == "md"),
