...
La part des salaires dans la valeur ajoutée nette est croissante en France [@fig-psal] (de 10 points de 1998 à 2025), comme en Espagne (de 9 points). Elle atteint en France le niveau le plus élevé des pays sélectionnés, pour autant que l'on puisse comparer entre pays. La part des salaires dans la valeur ajoutée est la plus basse aux Pays-Bas et est sur une pente décroisante depuis plus de 20 ans, alors qu'elle semble stable en Belgique et en Allemagne. L'italie affiche une importante variabilité temporelle.

Elle est cependant stable en France depuis 2020 (voire légèrement décroissante). Ce même mouvement s'observe dans les autres pays, plus nettement. La phase d'inflation à partir de 2022 a induit une moindre indexation des salaires [voir @fig-salaires] puis un rattrapage.

On détaille dans la suite de ce document les effets des corrections appliquées, ainsi que la différence entre les mesures dérivées des comptes de branche ou des comptes d'agents. On explore également les conséquences en matière de taux de profit (part des profits nets dans la valeur ajoutée) ou rendement du capital (profits nets divisés par les actifs).
...
data <- salaires |> 
  ...
  mutate(
    ...
    Salaire réel, branches marchandes hors imm.&fin. {round(wr_hifi,1)} k€/an
...
labels_branches <- c(wr = "Ttes branches", wr_md = "Marchand", 
                     wr_nmd = "Non marchand", wr_hifi = "Marchand hors imm.&fin.")
sal <- ggplot(data) +
  aes(x=time, y=value, 
      alpha = name, linewidth = name, linetype = name,
      color = geo) +
  scale_alpha_manual(
    values = c(wr=1, wr_md = 1, wr_nmd = 0.75, wr_hifi = 0.75),
    labels = labels_branches, name = NULL) +
  scale_linetype_manual(
    values = c(wr="solid", wr_md ="solid", wr_nmd ="solid", wr_hifi = "11"),
    labels = labels_branches,
    name = NULL) +
  scale_linewidth_manual(
    values = c(wr=.5, wr_md = 0.1, wr_nmd = 0.25, wr_hifi = 0.5),
    labels = labels_branches, name = NULL) +
  geom_point_interactive(
    data = ~.x |> 
      filter( name == "wr") |>  
  ...
  guides(linetype = "legend", linewidth = "legend", color = "none", fill="none") +
  ofce_caption(
    wrap = 0,
...
psal <- ggplot(naq |> filter(champ %in% c("tb", "mdhifi"))) +
  aes(x=time, y=psal, col = geo, linetype = champ, alpha = champ) +
  scale_alpha_manual(values = c(tb=.25, md=.25, mdhifi = 1),
                     labels = labels_champ,
                     name = NULL) +
  scale_linetype_manual(values = c(tb="11", md="solid", mdhifi ="solid"),
                        labels = labels_champ,
                        name = NULL) +
...
psal <- ggplot(naq |> filter(champ %in% c("tb", "mdhifi"))) +
  ...
  scale_linetype_manual(values = c(tb="11", md="solid", mdhifi ="solid"),
                        labels = labels_champ,
                        name = NULL) +
  geom_ribbon(aes(ymin = psalb, ymax = psal, fill = geo), 
              color = NA, alpha=0.1, show.legend = FALSE) +
  geom_line_interactive( 
    aes(y=psalb, 
        tooltip = "Part des salaires dans la VAB, corrigée de la non salarisation",
        data_id = champ),
    linewidth = 0.2) +
  geom_line_interactive(
    aes(tooltip = "Part des salaires dans la VAN, corrigée de la non salarisation",
...
psnf <- ggplot(snfq |> filter(time >= "1995-01-01")) +
  ...
  geom_line_interactive(
    data=naq |> filter(champ == "mdhifi"), 
    aes(y=psal, 
        tooltip = "mesurée par les comptes de branches (marchand hors finance)"),
    alpha = 0.75, linewidth = 0.25, linetype = "11") +
  geom_ribbon(
    aes(ymin = psalb, ymax =psal, fill = geo), alpha =0.1, color = NA) +
  geom_line_interactive(
     aes(y=psalb, tooltip = "Part des salaires dans la VAB, SNF"),
     alpha = 0.5, linewidth = 0.33) +
   geom_line_interactive(
  ...
  geom_point_interactive(
     aes( y = psal, fill = geo,
        ...
        size = 1, stroke = 0.1, shape = 21, color = "white",
        hover_nearest = TRUE, show.legend=FALSE) +
  ylab("% de la VA") + xlab(NULL) +
  facet_wrap(vars(geo), ncol = 3, labeller = lbl) +
  scale_ofce_date(
...
## Profits nets et dividendes dans les comptes d'agent
