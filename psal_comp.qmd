...
Depuis 2018, en France, la part des salaires est stabilisée, à un haut niveau. L'inflation et le retard d'ajustement des salaires sur l'inflation explique probablement cette trajectoire. On observe des mouvements comparables dans d'autres pays, bien que plus violent en Allemagne ou en Italie par exemple.
...
naq_full |> 
  ...
  filter( year %in% c(1995, 2025), champ == "mdhifi" ) |>
  select(year, geo, psal, van) |> 
  pivot_wider(names_from = year, values_from = c(psal,van)) |> 
  ggplot()+aes(x=psal_2025, y=psal_1995, size = van_2025)+geom_point() + geom_text(aes(label=geo), color = "white", size = 9, size.unit = "pt", )+
  geom_abline(slope = 1, intercept = 0)+scale_size(range = c(4, 8))
```
