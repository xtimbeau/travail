...
psal <- imap(c("hors services immobiliers (-L)" = "mdhi", "hors loyers imputés des ménages (-L68A)" = "mdhim"),
 ...
 comptes nationaux trimestriels (namq_10_a10), téléchargés le {date}, code à github.com/xtimbeau/travail/vaq.r.",
    ...
    note = "La CCF ou les impôts nets de subventions proviennent des comptes annuels, jusqu'en 2023 ou 2024 selon les pays, extrapolée au delà (voir tableau A1). Le partage de la valeur ajoutée pour les tous derniers points évolue donc comme le partage dans la valeur ajoutée brute.") ) 
...
psal <- ggplot(data) +
  ...
  scale_linewidth_manual(
    ...
    name = NULL) +
  geom_ribbon(
    data = naq |> filter(champ %in%c("mdhim", "tb")),
...
```{r, results="asis"}
#| label: tbl-rang
#| tbl-cap: Part des salaires dans la VA et rang, différents concepts 
